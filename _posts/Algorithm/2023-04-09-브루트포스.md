...
```
임의로 50개의 데이터를 만들어 돌려보았을 때는 충분히 시간안에 동작하는 것처럼 보였다. 하지만 제출해보니 시간 초과가 나왔다. 이럴 경우 이 알고리즘을 개선하거나 아니면 이 알고리즘을 짜면서 새롭게 얻은 영감으로 새로운 알고리즘을 짜는 두 가지 방법이 있다. 나는 두번째 방법을 이용해 보았다.  
``` c++
#include <iostream>
#include <vector>
